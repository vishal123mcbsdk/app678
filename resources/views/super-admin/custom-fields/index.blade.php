...
<script>
    $(function() {
        var table = $('#custom_fields').dataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{!! route('super-admin.custom-fields.data') !!}',
            "order": [[ 0, "desc" ]],
            deferRender: true,
            language: {
        ...
        $('body').on('click', '.sa-params', function(){
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    var url = "{{ route('super-admin.custom-fields.destroy',':id') }}";
...
//                                    swal("Deleted!", response.message, "success");
        ...
        $('#add-field').click(function(){
            var url = '{{ route('super-admin.custom-fields.create')}}';
        ...
        $('body').on('click', '.edit-custom-field', function() {
            var id = $(this).data('user-id');
            var url = "{{ route('super-admin.custom-fields.edit',':id') }}";
            url = url.replace(':id', id);
            $('#modelHeading').html('Manage Project Category');
            $.ajaxModal('#projectCategoryModal',url);
        })
