...
<div class="modal-header">
    ...
    <h4 class="modal-title"><i class="icon-clock"></i> @lang('app.menu.attendance') @lang('app.details') </h4>
        @if ($user_attendance->half_day == 'yes')
            <span class="label label-success half-day"><i class="fa fa-check"></i> @lang('modules.attendance.halfDay')</span>
        @endif
        @if ($user_attendance->late == 'yes') 
        <span class="label label-danger half-day"><i
                    class="fa fa-check"></i> @lang('modules.attendance.late')</span>
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card punch-status">
                <div class="white-box">
                    <h4>@lang('app.menu.attendance') <small
                            class="text-muted">{{ $startTime->format($global->date_format) }}</small> ({{ $user_attendance->working_from }})</h4>
                    <div class="punch-det">
                ...
                <div class="white-box">
                    ...
                    @foreach ($attendanceActivity->reverse() as $item)
                        <div class="row res-activity-box" id="timelogBox{{ $item->aId }}">
                            <ul class="res-activity-list col-md-9">
                                <li>
                                    ...
                                    <p class="res-activity-time">
                                        ...
                                        <i class="fa fa-map-marker m-l-5"></i>
                                        {{ ucwords($item->working_from) }}
                                    </p>
                                </li>
                                <li>
                            ...
                            <div class="col-md-3">
                                <a href="javascript:;" onclick="editAttendance({{ $item->aId }})"
                                    style="display: inline-block;" id="attendance-edit"
                                    data-attendance-id="{{ $item->aId }}"><label class="label label-info"><i
                                            class="fa fa-pencil"></i> </label></a>
                                <a href="javascript:;" onclick="deleteAttendance({{ $item->aId }})"
                                    style="display: inline-block;" id="attendance-edit"
                                    data-attendance-id="{{ $item->aId }}"><label class="label label-danger"><i
                                            class="fa fa-times"></i></label></a>
                            </div>
                        </div>
                    @endforeach
