@forelse($lead->follow as $follow)
    <a href="javascript:;" data-follow-id="{{ $follow->id }}" id="followUpBox{{ $follow->id }}"  class="list-group-item edit-task">
        <h4 class="list-group-item-heading sbold">@lang('app.createdOn'): {{ $follow->created_at->toFormattedDateString() }} <button type="button" title="@lang('app.delete')" onclick="removeFollow({{ $follow->id }});" class="btn btn-danger float-right followup-remove"><i class="fa fa-remove"></i></button></h4>
